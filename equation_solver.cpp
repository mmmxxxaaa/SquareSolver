...
static float make_discriminant(struct quadric_coeffs coeffs);
...
static enum PossibleSolutionCases solve_normal_quadratic_equation(float * ptr_x1, float * ptr_x2, int * ptr_amount_of_roots, struct quadric_coeffs coeffs);
...
enum PossibleSolutionCases general_solution(struct answer_and_solution * result, struct quadric_coeffs coeffs)
{
    assert(result != NULL);
